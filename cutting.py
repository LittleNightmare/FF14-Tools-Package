...
def blue_pos(a_im):
    ...
    for y in range(START_POINT[1], END_Y + 1):
        x = pow(pow(START_POINT[0] - CENTER[0], 2) + pow(START_POINT[1] - CENTER[1], 2) - pow(y - CENTER[1], 2), 0.5) + \
            CENTER[0]
        x = round(x)
        x = x - CROP[0]
        y = y - CROP[1]
...
def start_cutting():
    ...
    while 1:
        ...
        END_Y = 813
        END_X = round(
            pow(pow(START_POINT[0] - CENTER[0], 2) + pow(START_POINT[1] - CENTER[1], 2) - pow(END_Y - CENTER[1], 2),
                0.5) + CENTER[0])
